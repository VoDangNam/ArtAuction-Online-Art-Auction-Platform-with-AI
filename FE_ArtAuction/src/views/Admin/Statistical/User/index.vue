  ...
  <div class="card border-0 shadow-sm h-100">
    ...
    <div class="card-body px-4 pb-4">
      <div v-if="!startDate || !endDate" class="text-center py-5 bg-light text-muted rounded">
        ...
        <p>Vui lòng chọn khoảng thời gian để xem thống kê người dùng</p>
      </div>
...
import { getUsersRegistrationStats } from "@/services/statisticsService";
...
export default {
  ...
  methods: {
    ...
    async updateChart() {
      ...
      try {
        const response = await getUsersRegistrationStats(this.startDate, this.endDate);
        ...
        if (response && response.status === 1) {
          ...
          this.error = response?.message || "Không thể tải dữ liệu người dùng";
          this.loading = false;
        }
      } catch (err) {

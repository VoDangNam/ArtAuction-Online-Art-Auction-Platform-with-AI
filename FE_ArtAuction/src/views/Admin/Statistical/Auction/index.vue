<template>
  <div class="card border-0 shadow-sm h-100">
    ...
    >
      <div>
        <h5 class="fw-bold text-warning mb-1">Auction Rooms</h5>
        <p class="text-muted small mb-0">Phòng đấu giá được mở</p>
      </div>
    ...
    <div class="card-body px-4 pb-4">
      <div v-if="!startDate || !endDate" class="text-center py-5 bg-light text-muted rounded">
        ...
        <p>Vui lòng chọn khoảng thời gian để xem thống kê phòng đấu giá</p>
      </div>
...
import { getAuctionRoomsStats } from "@/services/statisticsService";
...
export default {
  ...
  methods: {
    ...
    async updateChart() {
      ...
      try {
        const response = await getAuctionRoomsStats(this.startDate, this.endDate);
        ...
        if (response && response.status === 1) {
          ...
          this.error = response?.message || "Không thể tải dữ liệu phòng đấu giá";
          this.loading = false;
        }
      } catch (err) {

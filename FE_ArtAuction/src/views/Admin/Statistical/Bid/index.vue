  ...
  <div class="card border-0 shadow-sm h-100">
    ...
    >
      <div>
        ...
        <p class="text-muted small mb-0">Lượt đặt giá theo thời gian</p>
      </div>
    ...
    <div class="card-body px-4 pb-4">
      <div v-if="!startDate || !endDate" class="text-center py-5 bg-light text-muted rounded">
        ...
        <p>Vui lòng chọn khoảng thời gian để xem thống kê lượt bid</p>
      </div>
...
import { getBidsStats } from "@/services/statisticsService";
...
export default {
  ...
  methods: {
    ...
    async updateChart() {
      ...
      try {
        const response = await getBidsStats(this.startDate, this.endDate);
        ...
        if (response && response.status === 1) {
          ...
          this.error = response?.message || "Không thể tải dữ liệu lượt bid";
          this.loading = false;
        }
      } catch (err) {
            ...
            tooltip: {
              callbacks: {
                label: function (context) {
                  return ` ${context.parsed.y} lượt bid`;
                },
              },
            },
          },
